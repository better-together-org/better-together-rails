...
RSpec.describe VenueBuilding, type: :model do
  ...
  describe 'callback #set_new_building_details' do
    ...
    context 'when creating a new venue_building' do
      ...
      before { venue_building.valid? }

      it 'copies parent attribute name from venue' do
        expect(venue_building.building.name).to eq('Test Venue')
      end

      it 'copies parent attribute description from venue' do
        expect(venue_building.building.description.body).to eq(venue.description.body)
      end

      it 'copies parent attribute privacy from venue' do
